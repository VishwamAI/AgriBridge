...
const OrderManagement = () => {
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    setError('');
    try {
      const response = await axios.post(`${API_BASE_URL}/generate-rider-otp`, { orderId }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      setOtp(response.data.otp);
      setShowOTP(true);
      setSelectedOrder(prevOrder => ({ ...prevOrder, pendingStatus: newStatus }));
    } catch (err) {
  ...
  const handleOTPVerify = async (enteredOtp) => {
    ...
    try {
      ...
      await axios.post(`${API_BASE_URL}/update-order-status`, {
      ...
      setOrders(prevOrders => prevOrders.map(order =>
        order.id === selectedOrder.id ? { ...order, status: selectedOrder.pendingStatus } : order
      ));
      setShowOTP(false);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <h3 className="text-xl font-semibold mb-2">Current and Upcoming Deliveries</h3>
          {orders.map(order => (
