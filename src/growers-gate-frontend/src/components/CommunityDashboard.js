    ...
    <div>
      ...
      <div className="flex justify-between mb-4">
        {checkoutSteps.map((step, index) => (
          <div
            key={step}
            className={`${currentStep === step ? "font-bold" : ""}`}
          >
...
const PaymentManagement = () => {
  ...
  const handleAddPaymentMethod = (e) => {
    ...
    setPaymentMethods([
      ...paymentMethods,
      { ...newPaymentMethod, id: paymentMethods.length + 1 },
    ]);
      ...
      <ul className="mb-4">
        {reviews.map((review) => (
          <li key={review.id} className="mb-2">
            Product ID: {review.productId}, Rating: {review.rating}, Comment:{" "}
            {review.comment}
          </li>
        ))}
      </ul>
      ...
      const updatedForums = forums.map((forum) =>
        forum.id === selectedForum.id
          ? {
              ...
              posts: [
                ...forum.posts,
                { ...newPost, id: forum.posts.length + 1 },
              ],
            }
          : forum,
      );
